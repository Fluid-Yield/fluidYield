...
contract Strategy is Ownable2Step {
    ...
    mapping(address => IFluidStrategy.Strategy[]) curatorStrategies;

    // strategy ID => strategies
    mapping(bytes32 => IFluidStrategy.Strategy) strategies;

    // strategyId => stats
    mapping(bytes32 => IFluidStrategy.StrategyStats) strategyStats;

    // strategyId => user => userStats
    mapping(bytes32 => mapping(address => IFluidStrategy.UserStats)) userStats;
    ...
    function createStrategy(
        ...
        IFluidStrategy.Step[] memory _steps,
        uint256 _minDeposit
    ) public {
        ...
        IFluidStrategy.Strategy memory _strategy = IFluidStrategy.Strategy({
        ...
        IFluidStrategy.StrategyStats storage stats = strategyStats[strategyId];
    ...
    function updateUserStats(
        ...
        // Get user's stats
        IFluidStrategy.UserStats storage _userStats = userStats[_strategyId][_userAddress];

        IFluidStrategy.AssetBalance memory tempAssetBal =
            IFluidStrategy.AssetBalance({assets: _assets, amounts: _assetsAmount});

        IFluidStrategy.ShareBalance memory tempShareBal = IFluidStrategy.ShareBalance({
            protocol: _protocol,
        ...
        if (_userStats.tokenBalances.length == stepIndex) {
            ...
            IFluidStrategy.AssetBalance memory userTokenBalances = _userStats.tokenBalances[stepIndex];
            IFluidStrategy.ShareBalance memory userShareBalances = _userStats.shareBalances[stepIndex];
    ...
    function updateStrategyStats(
        ...
        IFluidStrategy.StrategyStats storage stats = strategyStats[_strategyId];
     ...
     * @param _strategyId strategy identity
     */
    function getStrategy(bytes32 _strategyId) public view returns (IFluidStrategy.Strategy memory) {
        IFluidStrategy.Strategy memory strategy = strategies[_strategyId];
     ...
     */
    function getStrategy(address _curator) public view returns (IFluidStrategy.Strategy[] memory) {
     ...
     */
    function getAllStrategies() public view returns (IFluidStrategy.Strategy[] memory) {
        uint256 length = allStrategyIds.length;
        IFluidStrategy.Strategy[] memory allStrategies = new IFluidStrategy.Strategy[](length);
    ...
    function getUserStrategyStats(bytes32 _strategyId, address _user)
        public
        view
        returns (IFluidStrategy.UserStats memory)
    {
    ...
    function getUserAssetBalance(bytes32 _strategyId, address _user, address[] memory _assets, uint256 _stepIndex)
        public
        view
        returns (IFluidStrategy.AssetBalance memory)
    {
        IFluidStrategy.UserStats memory stats = userStats[_strategyId][_user];
        ...
        return IFluidStrategy.AssetBalance(_assets, new uint256[](0));
    }
    ...
    function getUserShareBalance(
        ...
        address _shareToken,
        uint256 _stepIndex
    ) public view returns (IFluidStrategy.ShareBalance memory) {
        IFluidStrategy.UserStats memory stats = userStats[_strategyId][_user];
        ...
        return IFluidStrategy.ShareBalance(_protocol, _shareToken, 0, new address[](0), new uint256[](0));
    }
     ...
     * @param steps array representing the individual steps involved in the strategy
     */
    function _validateSteps(IFluidStrategy.Step[] memory steps) internal view returns (bool) {
