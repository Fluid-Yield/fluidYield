...
contract Engine is ERC4626 {
    ...
    constructor() ERC4626(IERC20(address(this))) ERC20("Fluid", "FLUID") {}

    function join(bytes32 _strategyId, address _strategyModule, uint256[] memory _amounts) public {
        // Fetch the strategy
        IFluidStrategy.Strategy memory strategy = IFluidStrategy(_strategyModule).getStrategy(_strategyId);
        ...
        for (uint256 i; i < strategy.steps.length; i++) {
            // Fetch step
            IFluidStrategy.Step memory step = strategy.steps[i];
            ...
            try IConnector(step.connector).execute(
                ...
                // update user info
                IFluidStrategy(_strategyModule).updateUserStats(
                    _strategyId,
                    msg.sender,
                    protocol,
                    assets,
        ...
        IFluidStrategy(_strategyModule).updateStrategyStats(
            _strategyId, strategy.steps[0].assetsIn, _amounts, msg.sender, _fee, 0
        );

        // update user's strategy array
        IFluidStrategy(_strategyModule).updateUserStrategy(_strategyId, msg.sender, 0);

        // update the user's joined status
        IFluidStrategy(_strategyModule).setJoinedStrategy(_strategyId, msg.sender, true);
    ...
    function exit(bytes32 _strategyId, address _strategyModule) public {
        // check and burn user's fluid share token (also prevent re-enterancy)

        // Fetch the strategy
        IFluidStrategy.Step[] memory steps = IFluidStrategy(_strategyModule).getStrategy(_strategyId).steps;

        // Execute all steps in reverse atomically
        for (uint256 i = steps.length; i > 0; i--) {
            // Fetch step
            IFluidStrategy.Step memory step = steps[i - 1];
        ...
        IFluidStrategy.AssetBalance memory userAssetBalance =
            IFluidStrategy(_strategyModule).getUserAssetBalance(_strategyId, msg.sender, steps[0].assetsIn, 0);
        ...
        IFluidStrategy(_strategyModule).updateUserStrategy(_strategyId, msg.sender, 1);

        // update the user's joined status
        IFluidStrategy(_strategyModule).setJoinedStrategy(_strategyId, msg.sender, false);
